...
class PayDirectProductMerchantInterface extends StatefulWidget {
  const PayDirectProductMerchantInterface({Key? key}) : super(key: key);

  @override
  _PayDirectProductMerchantInterfaceState createState() => _PayDirectProductMerchantInterfaceState();
}

class _PayDirectProductMerchantInterfaceState extends State<PayDirectProductMerchantInterface>
...
class PayDirectCheckoutScreen extends StatefulWidget {
  const PayDirectCheckoutScreen({Key? key}) : super(key: key);

  @override
  _PayDirectCheckoutScreenState createState() => _PayDirectCheckoutScreenState();
}

class _PayDirectCheckoutScreenState extends State<PayDirectCheckoutScreen> with TickerProviderStateMixin {
  late AnimationController _fadeController;
  late Animation<double> _fadeAnimation;
  ...
  void _submitForm(BuildContext context, double totalAmount, String currency) {
    if (_formKey.currentState!.validate()) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Payment submitted: ${payload.toString()}',
          ),
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        shadowColor: Colors.black.withOpacity(0.1),
        title: Text(
          'UrbanThread - PayDirect Checkout',
          style: GoogleFonts.playfairDisplay(
            fontSize: isLargeScreen ? 28 : 24,
            fontWeight: FontWeight.w700,
            color: const Color(0xFF2B2B2B),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: ElevatedButton(
                          onPressed: () => _submitForm(context, totalAmount, currency),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: const Color(0xFF2B2B2B),
                            foregroundColor: Colors.white,
                            shape: RoundedRectangleBorder(
